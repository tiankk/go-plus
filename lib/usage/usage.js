...
class Usage {
  ...
  parse (obj) {
    ...
    const parsed = parseGoPosition(obj[0].objpos)
    if (parsed) {
      result.set("refer to:", [{filename: parsed.file, row: parsed.line, column: parsed.column, text: obj[0].desc}])
    }
    for (const pkg of obj.slice(1)) {
      ...
      if (result.has(pkg.package)) {
        result.set(pkg.package, result.get(pkg.package).concat(refs))
      } else {
